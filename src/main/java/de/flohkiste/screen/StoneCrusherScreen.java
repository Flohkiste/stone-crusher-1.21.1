...
public class StoneCrusherScreen extends HandledScreen<StoneCrusherScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        final Function<Identifier, RenderLayer> RENDER_LAYER = RenderLayer::getText;
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.setShaderTexture(0, TEXTURE);
        int x = (width - backgroundWidth) / 2;
        int y = (height - backgroundHeight) / 2;
        context.drawTexture(RENDER_LAYER, TEXTURE, x, y, 0, 0, backgroundWidth, backgroundHeight);
